from src.documents.chunking import _looks_like_heading, chunk_docs
...
def test_chunk_docs_splits_large_paragraphs_and_deduplicates_content():
    ...
    chunks = chunk_docs([_doc(text), duplicate], workspace_id="wid", chunk_size=18, chunk_overlap=4)

    assert len(chunks) >= 3
    assert sum(1 for c in chunks if c.content == "重复内容。") == 1
